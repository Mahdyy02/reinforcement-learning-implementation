...
Q = {(state,action):compute_q_value(state,action) for state in range(env.observation_space.n) for action in range(env.action_space.n)}

data_str_keys = {json.dumps(k): v for k, v in Q.items()}
